  ...
  <view class="orders-page">
    ...
    <view class="stats-section">
      <view class="stats-item" @tap="filterOrders('pending_pay')">
        <text class="stats-value">{{ orderStats.pendingPayment }}</text>
        <text class="stats-label">待付款</text>
      </view>
      <view class="stats-item" @tap="filterOrders('pending_ship')">
        <text class="stats-value">{{ orderStats.toShip }}</text>
        <text class="stats-label">待发货</text>
      </view>
      <view class="stats-item" @tap="filterOrders('pending_recv')">
        <text class="stats-value">{{ orderStats.toReceive }}</text>
        <text class="stats-label">待收货</text>
      </view>
      <view class="stats-item" @tap="filterOrders('refunding')">
        <text class="stats-value">{{ orderStats.afterSale }}</text>
        <text class="stats-label">待售后</text>
      </view>
      <view class="stats-item" @tap="filterOrders('completed')">
        <text class="stats-value">{{ orderStats.completed }}</text>
    ...
    >
      ...
      <view v-if="filteredOrders.length === 0" class="empty-state">
        <text class="iconfont icon-dingdanxiaoxi" style="font-size: 120rpx; opacity: 0.3; color: #ddd; display: block; margin-bottom: 20rpx;"></text>
        <text>暂无订单</text>
      </view>
...
const searchOrders = () => {
  ...
  console.log('[商家订单搜索] 搜索关键词:', searchKeyword.value)
}
...
const confirmShip = async () => {
  ...
  if (logisticsType === 0 && (!trackingNum || !express_company)) {
    uni.showToast({ title: '请选择快递公司并填写物流单号', icon: 'none' })
    return
  }
...
const loadOrders = async (append = false) => {
  try {
    ...
    let processedOrders = await Promise.all(orderList.map(async order => {
      ...
      // 优化：先尝试从订单数据中获取图片，如果没有则通过API获取，但使用异步方式不阻塞主流程
      let products = []
